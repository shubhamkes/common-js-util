...
function InitializeMemory() {
    ...
    const GetItem = Getter(MAPPER.GetItem);
    MemoryStore = GetItem('memoryStore') || {}; // memory store would ba as same as Store except that is would store its data in asynStorage 
}
...
export function StoreEvent({ eventName, data, objParams, isMemoryStore }) {
    InitializeMemory();
    const SetItem = Getter(MAPPER.SetItem);
