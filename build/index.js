 ...
 */
function SelectFromOptions(hayStack, needle, element, defaultElement, shouldNotSendDefaultElement) {
    defaultElement = defaultElement || 0;
    var isArray = IsUndefinedOrNull(element);
    ...
    if (!shouldNotSendDefaultElement) {
        var finalElement = hayStack[defaultElement];
...
function ArrayToObject(key) {
    ...
    array.forEach(function (element, index) {
        if (!key) {
            obj[index] = element;
        } else if (element && (typeof element === 'undefined' ? 'undefined' : _typeof(element)) == 'object' && element[key]) {
...
function IsEqualObject(object, otherObject) {
    return _lodash2.default.isEqual(object, otherObject);
}
 ...
 */
function TrimQueryString(options) {
 ...
 * @param  {object} params
 */
function BuildUrlForGetCall(url, params) {
    var newUrl = url + "?";
    for (var i in params) {
        var value = params[i];
        newUrl += i + "=" + value + "&";
